...
{
    float4 originalColor = tex.Sample(samp, ps_in.uv) * ps_in.color;

    // If we're inside the sprite, return original color
    ...
    // If any surrounding pixel has alpha, we're at an edge
    if (alpha > 0.0)
    {
